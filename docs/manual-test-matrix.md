...
| Edit mode (default: Shift + Right ⌥): select text in Notes, hold the chord, say "make this formal" → selection replaced | |
| Edit mode preset "Right ⌥ alone": switch in Settings → General, confirm plain Right ⌥ still triggers edit mode | |
| Edit mode preset "Shift + Right ⌘" with Dictation also on Right ⌘: hold the chord → edit mode wins (brief recording flash acceptable), selection replaced | |
